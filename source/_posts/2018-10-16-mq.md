...
广播有时也叫pub和sub，就是一个发布者，多个订阅者。每个订阅者都会收到相同的消息，消息被多次消费。
点对点，一个发布者对应一个消费者，消息只会被消费一次
...
- Work Queue 工作队列，一个队列被多个消费者消费，且一个消息只消费一次![](https://www.rabbitmq.com/img/tutorials/python-two.png)
- Publish/Subscribe 发布订阅，通过exchange进行队列分发，实现一条消息可以被多个消费者消费![](https://www.rabbitmq.com/img/tutorials/python-three.png)
